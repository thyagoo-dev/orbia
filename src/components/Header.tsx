import { LogOut, Moon, Orbit, Sun } from 'lucide-react'
    ...
    <header className="orbia-header mx-auto flex w-full max-w-[1680px] items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-baseline gap-2">
            <h1 className="text-xl font-black tracking-tight text-ink">Orbia</h1>
            <span className="hidden text-xs font-semibold text-muted sm:inline">beta pessoal</span>
          </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <div className="hidden text-right sm:block">
          ...
          <p className="max-w-52 truncate text-xs text-muted">{user?.email}</p>
        </div>
        <button
        ...
        <button className="icon-button" onClick={() => void signOut()} aria-label="Sair" title="Sair">
          <LogOut size={17} />
        </button>
      </div>
    </header>
